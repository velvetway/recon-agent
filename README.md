...
Наблюдения прогона сопоставляются с кандидатами-CWE по детерминированному
словарю правил `data/rules/*.yaml`. Это не утверждение об уязвимости, а
вектор для проверки. Каждый кандидат проходит **CWE-guard**: принимается,
только если его ID есть в каталоге — правило с опечаткой в CWE не загрузится.

```bash
make map RUN=latest        # или ./bin/agent -map latest
```
...
```
• db.demo.test
    CWE-306  0.40  Отсутствие аутентификации для критической функции
      сигнал: open_port=3306/tcp [m1]  правило: exposed-datastore
      вектор: Проверить доступ без аутентификации и дефолтные учётные данные…
```

Правило — это условие по наблюдению (`kind`, `tool`, `value_contains`,
`value_regex`, `evidence_*`) → CWE, `confidence` и вектор проверки. Стартовый
набор покрывает открытые СУБД, FTP/Telnet, RDP, панели управления, раскрытие
версии сервера и стек WordPress; словарь пополняется. LLM (фаза 7) расширит и
приоритизирует список, а не заменит его. Запись рёбер
`(:Service)-[:POTENTIALLY_VULN_TO]->(:CWE)` в граф и отчёт — фаза 8.
